...
function parse(provider, scheme){
  var mirrorScheme = _.pick(scheme, _.isString), json = {}, operation;
  var depth = _.omit(scheme, notToParse);
  _.forOwn(depth, function(subScheme, schemeKey) {
    if(isOperation(schemeKey)){
      //console.log(subScheme, _.keys(_.pick(subScheme, _.isString)).length, _.keys(subScheme).length);
...
function executeOperation(operation, provider, scheme){
  ...
  if(operation.name === 'mirror'){
    /*var items;
    if(operation.args){
      console.log('operation.args', operation.args);
      items = _.uniq(provider, operation.args);
    } else {
      items = provider;
    }
    console.log('items', items);
    var children = _.map(items, function(item){
      console.log('item', item, scheme, parse([item], scheme));
...
function notToParse(value){
  return _.isString(value) || _.isFunction(value);
};
